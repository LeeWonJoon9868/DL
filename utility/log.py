...
class Log:
    def __init__(self, log_each: int, initial_epoch=-1, filename='./log/log1.csv'):
    ...
    def flush(self) -> None:
        if self.is_train:
            ...
            self.train_acc = self.epoch_state["accuracy"] / self.epoch_state["steps"]
            self.time = self._time()
            print(
                f"\r┃{self.epoch:12d}  ┃{self.train_loss:12.4f}  │{100*self.train_acc:10.2f} %  ┃{self.learning_rate:12.3e}  │{self.time:>12}  ┃",
                end="",
                flush=True,
            )

        else:
            self.test_loss = self.epoch_state["loss"] / self.epoch_state["steps"]
            self.test_acc = self.epoch_state["accuracy"] / self.epoch_state["steps"]

            print(f"{self.test_loss:12.4f}  │{100*self.test_acc:10.2f} %  ┃", flush=True)

            if self.test_acc > self.best_accuracy:
                self.best_accuracy = self.test_acc

            newrow = [self.epoch, self.train_loss, 100*self.train_acc, self.learning_rate, self.time, self.test_loss, 100*self.test_acc]
            self.appendNewRow(elementsToAppend=newrow)
    ...
    def _print_header(self) -> None:
        ...
        newrow = ['epoch', 'train_loss', 'train_accuracy', 'lr', 'elapsed', 'valid_loss', 'valid_accuracy']
        self.appendNewRow(elementsToAppend=newrow)

    def appendNewRow(self, elementsToAppend):
    # open the csv file in the append mode
        with open(self.filename, 'a+', newline='') as append_obj:
